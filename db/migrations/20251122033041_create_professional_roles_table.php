 ...
 * 
 * Esta migration cria:
 * 1. professional_roles - Roles específicas da empresa (Admin, Gerente, Profissional, Atendente, etc.)
...
final class CreateProfessionalRolesTable extends AbstractMigration
{
    ...
    {
        // 1. Tabela: professional_roles (idempotente)
        if (!$this->hasTable('professional_roles')) {
            $roles = $this->table('professional_roles', [
                'id' => 'id',
                'engine' => 'InnoDB',
                'charset' => 'utf8mb4',
                'collation' => 'utf8mb4_unicode_ci',
                'comment' => 'Roles específicas dos profissionais da empresa'
            ]);
            $roles->addColumn('tenant_id', 'integer', ['signed' => false, 'null' => false])
                  ->addColumn('name', 'string', ['limit' => 100, 'null' => false, 'comment' => 'Nome da role (ex: Profissional, Atendente)'])
            ...
            $this->output->writeln('AVISO: Tabela professional_roles já existe. Pulando criação.');
        }

        // 2. Adiciona professional_role_id na tabela professionals (idempotente)
        $professionals = $this->table('professionals');
        if (!$professionals->hasColumn('professional_role_id')) {
            ...
            $this->output->writeln('AVISO: Campo professional_role_id já existe em professionals. Pulando adição.');
        }
    }
