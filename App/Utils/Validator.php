...
class Validator
{
    ...
    public static function validateRegister(array $data): array
    {
        ...
        // company_name: obrigatório, nome da empresa/tenant
        if (!isset($data['company_name'])) {
            $errors['company_name'] = 'Obrigatório';
        } elseif (!is_string($data['company_name'])) {
            $errors['company_name'] = 'Deve ser uma string';
        } else {
            $companyName = trim($data['company_name']);
            if (empty($companyName)) {
                $errors['company_name'] = 'Obrigatório';
            } elseif (strlen($companyName) > 255) {
                $errors['company_name'] = 'Muito longo (máximo 255 caracteres)';
            } elseif (strlen($companyName) < 3) {
                $errors['company_name'] = 'Muito curto (mínimo 3 caracteres)';
            }
        }
        
        // company_slug: opcional, mas se fornecido deve ser válido
        if (isset($data['company_slug'])) {
            if (!is_string($data['company_slug'])) {
                $errors['company_slug'] = 'Deve ser uma string';
            } else {
                $slug = trim($data['company_slug']);
                if (!empty($slug) && !\App\Utils\SlugHelper::isValid($slug)) {
                    $errors['company_slug'] = 'Formato inválido. Use apenas letras minúsculas, números e hífens';
                }
            }
        }
