...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Gerencie permissões e acessos dos usuários</p>
        </div>
    </div>
        ...
        <div class="col-md-6 mb-4">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-primary" id="permissionsCountBadge">0</span>
                </div>
                <div class="card-body">
        ...
        <div class="col-md-6 mb-4">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-success" id="userPermissionsCountBadge" style="display: none;">0</span>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                    ...
                    <div id="userPermissionsContainer">
                        <div class="text-center py-5">
                            ...
                            <p class="text-muted mt-3">Selecione um usuário para ver suas permissões</p>
                        </div>
                    </div>
                </div>
            </div>
...
function renderPermissions() {
    ...
    const countBadge = document.getElementById('permissionsCountBadge');
    document.getElementById('loadingPermissions').style.display = 'none';
    list.style.display = 'block';
    
    // ✅ CORREÇÃO: Garante que permissions seja um array
    if (!Array.isArray(permissions) || permissions.length === 0) {
        list.innerHTML = '<li class="list-group-item text-muted text-center py-4">Nenhuma permissão disponível</li>';
        if (countBadge) countBadge.textContent = '0';
        return;
    }
    
    if (countBadge) {
        countBadge.textContent = formatNumber(permissions.length);
    }
    ...
    list.innerHTML = permissions.map(perm => {
        ...
        <li class="list-group-item d-flex justify-content-between align-items-center">
            <span><code class="text-primary">${escapeHtml(permName)}</code></span>
...
async function loadUserPermissions(skipCache = false) {
    ...
    if (!userId) {
        container.innerHTML = '<div class="text-center py-5"><i class="bi bi-person-check fs-1 text-muted"></i><p class="text-muted mt-3">Selecione um usuário para ver suas permissões</p></div>';
        currentUserPermissions = []; // ✅ CORREÇÃO: Limpa permissões quando nenhum usuário está selecionado
        const userPermissionsBadge = document.getElementById('userPermissionsCountBadge');
        if (userPermissionsBadge) userPermissionsBadge.style.display = 'none';
        return;
    }
    ...
    try {
        ...
        const userPermissionsBadge = document.getElementById('userPermissionsCountBadge');
        if (userPermissionsBadge) {
            if (userPermissions.length > 0) {
                userPermissionsBadge.textContent = formatNumber(userPermissions.length);
                userPermissionsBadge.style.display = 'inline-block';
            } else {
                userPermissionsBadge.style.display = 'none';
            }
        }
        ...
        container.innerHTML = `
            ...
            <div id="userPermissionsList">
                ${userPermissions.length === 0 ? 
                    '<div class="text-center py-4"><i class="bi bi-shield-x fs-1 text-muted"></i><p class="text-muted mt-2">Nenhuma permissão atribuída</p></div>' :
                    userPermissions.map(perm => {
                        const permName = typeof perm === 'string' ? perm : (perm.permission || perm.name || perm);
                        return `
                        <div class="d-flex justify-content-between align-items-center mb-2 p-2 border rounded">
                            <code class="text-primary">${escapeHtml(permName)}</code>
                            <button class="btn btn-sm btn-outline-danger" onclick="removePermission(${userId}, '${permName}')" title="Remover permissão">
