...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Rastreie e analise requisições completas do sistema</p>
        </div>
    </div>
    ...
    <div id="traceResult" style="display: none;">
        ...
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <span class="badge bg-primary" id="traceSummaryBadge">0</span>
            </div>
            <div class="card-body">
                <div class="row g-3" id="traceSummary">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex gap-2 align-items-center">
                    <span class="badge bg-primary" id="logsCountBadge" style="display: none;">0</span>
                    <div class="btn-group" role="group">
                        <button class="btn btn-sm btn-outline-primary" onclick="toggleView('table')" id="viewTableBtn">
                            <i class="bi bi-table"></i> Tabela
                        </button>
                        <button class="btn btn-sm btn-outline-primary" onclick="toggleView('timeline')" id="viewTimelineBtn">
            ...
            <div class="card-body">
                ...
                <div id="emptyTrace" class="text-center py-5" style="display: none;">
                    ...
                    <p class="text-muted">Não há logs relacionados a este trace.</p>
                </div>
            </div>
        </div>
    </div>
...
function displayTrace(trace) {
    // Exibe resumo com cards de KPI
    const summary = trace.summary || {};
    const totalLogs = trace.total_logs || 0;
    const auditLogs = trace.audit_logs_count || 0;
    const appLogs = trace.application_logs_count || 0;
    const avgTime = summary.average_response_time || 0;
    
    const summaryBadge = document.getElementById('traceSummaryBadge');
    if (summaryBadge) {
        summaryBadge.textContent = formatNumber(totalLogs);
    }
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start">
                        <div class="flex-grow-1">
                            ...
                            <h2 class="mb-0 fw-bold">${formatNumber(totalLogs)}</h2>
                        </div>
                        <div class="kpi-icon">
        ...
        <div class="col-md-3 col-sm-6">
            <div class="card kpi-card kpi-card-info">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start">
                        <div class="flex-grow-1">
                            <p class="text-muted mb-1 small fw-medium">Audit Logs</p>
                            <h2 class="mb-0 fw-bold">${formatNumber(auditLogs)}</h2>
                        </div>
                        <div class="kpi-icon">
        ...
        <div class="col-md-3 col-sm-6">
            <div class="card kpi-card kpi-card-success">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start">
                        <div class="flex-grow-1">
                            <p class="text-muted mb-1 small fw-medium">App Logs</p>
                            <h2 class="mb-0 fw-bold">${formatNumber(appLogs)}</h2>
                        </div>
                        <div class="kpi-icon">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start">
                        <div class="flex-grow-1">
                            ...
                            <h2 class="mb-0 fw-bold">${avgTime}ms</h2>
                        </div>
                        <div class="kpi-icon">
...
function displayLogsTable(logs) {
    ...
    logs.forEach((log, index) => {
        ...
        row.innerHTML = `
            ...
            <td>${typeBadge}</td>
            <td>
                <code class="text-muted small">${escapeHtml(log.endpoint || log.message || 'N/A')}</code>
            </td>
            <td>
                ${log.method ? `<span class="badge bg-secondary">${escapeHtml(log.method)}</span>` : (log.level ? `<span class="badge bg-${getLevelBadgeColor(log.level)}">${escapeHtml(log.level)}</span>` : '-')}
            </td>
            <td>${log.response_status ? getStatusBadge(log.response_status) : '-'}</td>
            <td>
                <div class="fw-medium">${log.response_time ? `${log.response_time}ms` : '-'}</div>
            </td>
            <td>
                ${log.id ? `
                    <div class="btn-group" role="group">
                        <button class="btn btn-sm btn-outline-primary" onclick="showLogDetails('${logType}', ${log.id})" title="Ver detalhes">
                            <i class="bi bi-eye"></i>
                        </button>
                    </div>
                ` : '-'}
