 ...
 * ✅ NOVO: Documentação OpenAPI adicionada
 */
...
class AuthController
{
    ...
    /**
     * ✅ REFATORAÇÃO: Construtor agora recebe UserRepository via injeção de dependência
     * Mantém compatibilidade: se não for passado, cria um novo
     */
    public function __construct(?UserRepository $userRepository = null)
    {
        // Se não for passado repository, cria um novo (compatibilidade com código existente)
    ...
    #[OA\Post(
        ...
        requestBody: new OA\RequestBody(
            ...
            content: new OA\MediaType(
                ...
                schema: new OA\Schema(
                    ...
                    properties: [
                        'email' => new OA\Property(type: 'string', format: 'email', example: 'admin@empresa.com'),
                        'password' => new OA\Property(type: 'string', format: 'password', example: 'senha123'),
                        'tenant_id' => new OA\Property(type: 'integer', example: 1)
                    ]
                )
            )
        ),
        responses: [
            new OA\Response(
                ...
                content: new OA\MediaType(
                    ...
                    schema: new OA\Schema(
                        type: 'object',
                        properties: [
                            'success' => new OA\Property(type: 'boolean', example: true),
                            'data' => new OA\Property(
                                type: 'object',
                                properties: [
                                    'session_id' => new OA\Property(type: 'string', example: 'abc123...'),
                                    'user' => new OA\Property(type: 'object'),
                                    'tenant' => new OA\Property(type: 'object'),
                                    'csrf_token' => new OA\Property(type: 'string')
                                ]
                            )
                        ]
                    )
            ...
            new OA\Response(response: 400, description: 'Dados inválidos', ref: '#/components/responses/ValidationError'),
            new OA\Response(response: 401, description: 'Credenciais inválidas', ref: '#/components/responses/Unauthorized'),
            new OA\Response(response: 429, description: 'Rate limit excedido', ref: '#/components/responses/RateLimit')
        ]
    )]
    public function login(): void
    {
        try {
            ...
            // ✅ SEGURANÇA: Gera token CSRF após login bem-sucedido
            $csrfToken = CsrfHelper::generateToken($sessionId);
            ...
            // Retorna dados do usuário, token de sessão e token CSRF
            ResponseHelper::sendSuccess([
                ...
                'csrf_token' => $csrfToken, // ✅ Token CSRF incluído na resposta do login
                'csrf_token_expires_in' => 1800 // 30 minutos
            ]);
        } catch (\Exception $e) {
    ...
    /**
     * Registro de tenant e primeiro usuário
     * POST /v1/auth/register
     * 
     * Body JSON:
     * {
     *   "company_name": "Nome da Empresa",
     *   "company_slug": "nome-empresa",  // opcional, será gerado automaticamente se não fornecido
    ...
    public function register(): void
    {
        try {
            ...
            $companySlug = isset($data['company_slug']) && !empty($data['company_slug']) 
                ? trim($data['company_slug']) 
                : null;
            $email = trim($data['email']);
            $password = $data['password'];
            ...
            if ($companySlug !== null) {
                if (!\App\Utils\SlugHelper::isValid($companySlug)) {
                    ResponseHelper::sendValidationError(
                        'Slug inválido',
                        ['company_slug' => 'Formato inválido. Use apenas letras minúsculas, números e hífens'],
                        ['action' => 'register']
                    );
                    return;
                }
                
                if ($this->tenantModel->slugExists($companySlug)) {
                    ResponseHelper::sendError(
                        409,
                        'Slug já existe',
                        ...
                        'SLUG_ALREADY_EXISTS',
                        [],
                        ['action' => 'register', 'slug' => $companySlug]
                    );
                    return;
                }
            }
            ...
            try {
                ...
                // 1. Cria tenant
                $tenantId = $this->tenantModel->create($companyName, $companySlug);
    ...
    /**
     * Obtém token CSRF para a sessão atual
     * GET /v1/auth/csrf-token
     * 
     * Retorna um token CSRF válido para uso em requisições que modificam dados.
    ...
    public function getCsrfToken(): void
    {
        try {
            ...
            if (!$sessionId) {
                ResponseHelper::sendUnauthorizedError(
                    'Sessão não encontrada. Faça login primeiro.',
            ...
            // Gera ou obtém token CSRF
            $token = CsrfHelper::getToken($sessionId);
            
            if (!$token) {
                // Se não existe, gera novo
                $token = CsrfHelper::generateToken($sessionId);
            }
