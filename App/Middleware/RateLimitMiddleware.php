...
class RateLimitMiddleware
{
    ...
    private ?\App\Services\TenantRateLimitService $tenantRateLimitService;
    ...
    public function __construct(RateLimiterService $rateLimiter, ?\App\Services\TenantRateLimitService $tenantRateLimitService = null)
    {
        $this->rateLimiter = $rateLimiter;
        $this->tenantRateLimitService = $tenantRateLimitService;
    }
    ...
    public function check(?string $endpoint = null, ?array $customLimits = null): bool
    {
        ...
        // ✅ NOVO: Tenta obter limites do tenant primeiro
        $tenantId = Flight::get('tenant_id');
        $tenantLimits = null;
        
        if ($tenantId !== null && $this->tenantRateLimitService !== null) {
            try {
                $tenantLimits = $this->tenantRateLimitService->getLimits(
                    $tenantId,
                    $endpoint ?? $requestUri,
                    $method
                );
            } catch (\Exception $e) {
                // ✅ SEGURANÇA: Em caso de erro, usa limites padrão
                Logger::warning("Erro ao buscar limites do tenant no middleware, usando limites padrão", [
                    'tenant_id' => $tenantId,
                    'error' => $e->getMessage()
                ]);
                $tenantLimits = null;
        ...
        // Se tenant tem limites configurados, usa eles; senão, usa limites padrão
        if ($tenantLimits !== null) {
            $limitPerMinute = $tenantLimits['limit_per_minute'];
            $limitPerHour = $tenantLimits['limit_per_hour'];
            
            Logger::debug("Usando limites customizados do tenant", [
                'tenant_id' => $tenantId,
                'endpoint' => $endpoint ?? $requestUri,
                'method' => $method,
            ...
            // Determina limite para este endpoint baseado no método e tipo (padrão)
