...
class StripeService
{
    ...
    /**
     * Retorna o cliente Stripe (para uso em outros services)
     */
...
$refundParams['amount'] = (int)$options['amount'];
    ...
    /**
     * Cria uma invoice (fatura) no Stripe
     * 
     * @param array $data Dados da invoice:
     *   - customer (obrigatório): ID do customer no Stripe
     *   - auto_advance (opcional): Se true, finaliza automaticamente (padrão: false)
     *   - collection_method (opcional): 'charge_automatically' ou 'send_invoice' (padrão: 'send_invoice')
     *   - description (opcional): Descrição da invoice
     *   - metadata (opcional): Metadados
     *   - days_until_due (opcional): Dias até vencimento (quando collection_method = 'send_invoice')
     ...
     *   - customer_id (obrigatório): ID do customer no Stripe
     *   - stripe_account_id (opcional): ID da conta Stripe Connect (para pagamentos na conta da clínica)
     *   - auto_advance (opcional): Se true, finaliza automaticamente
     *   - collection_method (opcional): 'charge_automatically' ou 'send_invoice'
    ...
    public function createInvoice(array $data): \Stripe\Invoice
    {
        try {
            ...
            if (empty($data['customer'])) {
                throw new \InvalidArgumentException("Campo customer é obrigatório");
            }
            ...
            if (isset($data['auto_advance'])) {
                $params['auto_advance'] = (bool)$data['auto_advance'];
            }
            ...
            if (isset($data['days_until_due'])) {
                $params['days_until_due'] = (int)$data['days_until_due'];
            }
            ...
            // Se tiver stripe_account_id, usa Stripe Connect (conta da clínica)
            $client = $this->client;
            if (!empty($data['stripe_account_id'])) {
                // Para Stripe Connect, precisa criar cliente na conta conectada
                // Por enquanto, cria invoice na conta conectada usando requestOptions
                $invoice = $this->client->invoices->create($params, [
            ...
            Logger::error("Erro ao criar invoice", [
    ...
    /**
     * Finaliza uma invoice (torna ela cobrável)
     * 
     * @param string $invoiceId ID da invoice no Stripe
     * @param array $options Opções:
     *   - auto_advance (opcional): Se true, cobra automaticamente após finalizar (padrão: true)
     * @return \Stripe\Invoice Invoice finalizada
     */
    public function finalizeInvoice(string $invoiceId, array $options = []): \Stripe\Invoice
    {
        try {
            $params = [];

            if (isset($options['auto_advance'])) {
                $params['auto_advance'] = (bool)$options['auto_advance'];
            } else {
                // Por padrão, auto_advance é true
                $params['auto_advance'] = true;
            }

            // Se tiver stripe_account_id, usa Stripe Connect
            if (!empty($options['stripe_account_id'])) {
                $invoice = $this->client->invoices->finalizeInvoice($invoiceId, $params, [
                    'stripe_account' => $options['stripe_account_id']
                ]);
            } else {
                $invoice = $this->client->invoices->finalizeInvoice($invoiceId, $params);
            }

            Logger::info("Invoice finalizada", [
                'invoice_id' => $invoiceId,
                'status' => $invoice->status,
                'auto_advance' => $params['auto_advance'],
                'stripe_account_id' => $options['stripe_account_id'] ?? null
            ]);

            return $invoice;
        } catch (ApiErrorException $e) {
            Logger::error("Erro ao finalizar invoice", [
                'invoice_id' => $invoiceId,
                'error' => $e->getMessage()
            ]);
            throw $e;
    ...
    public function deleteProduct(string $productId): \Stripe\Product
    {
        try {
            ...
            // Lista TODOS os preços associados ao produto (não apenas 1)
            $allPrices = [];
            $hasMore = true;
            $priceOptions = ['product' => $productId, 'limit' => 100];
            
            while ($hasMore) {
                $prices = $this->client->prices->all($priceOptions);
            ...
            // Se tiver preços, desativa todos primeiro
            if (count($allPrices) > 0) {
                // Desativa todos os preços
                foreach ($allPrices as $price) {
                    try {
                        $this->client->prices->update($price->id, ['active' => false]);
                    } catch (\Exception $e) {
                        Logger::warning("Erro ao desativar preço", [
                            'price_id' => $price->id,
                            'error' => $e->getMessage()
                        ]);
                    }
                ...
                // Tenta deletar o produto após desativar preços
                try {
                    $product = $this->client->products->delete($productId);
                    
                    Logger::info("Produto deletado após desativar preços", [
                        'product_id' => $productId,
                    ...
                    // Se não puder deletar (pode ter assinaturas ativas), apenas desativa
                    $product = $this->client->products->update($productId, [
                        'active' => false
                    ]);
                    
                    Logger::info("Produto desativado (não pode ser deletado - pode ter assinaturas)", [
                        'product_id' => $productId,
                        'prices_count' => count($allPrices)
                    ]);
                ...
                try {
                    ...
                    // Se não puder deletar, apenas desativa
                    $product = $this->client->products->update($productId, [
                        'active' => false
                    ]);
    ...
    {
        try {
            // Validações obrigatórias - aceita 'customer' ou 'customer_id'
            $customerId = $data['customer'] ?? $data['customer_id'] ?? null;
            if (empty($customerId)) {
                throw new \InvalidArgumentException("Campo customer ou customer_id é obrigatório");
            }
     ...
     */
    public function invalidatePricesCache(): void
    {
        try {
            $redis = \App\Services\CacheService::getRedisClient();
            if ($redis) {
